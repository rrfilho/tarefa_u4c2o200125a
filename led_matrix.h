...
static void draw_frame(char frame[PIXELS][COLOR]) {
...
void led_matrix_draw_animation(char animation[FRAMES][PIXELS][COLOR]) {
    for (uint i = 0; i < FRAMES; i++) {
        draw_frame(animation[i]);
        sleep_ms(150);
    }
}

void led_matrix_draw(char hex[COLOR]) {
    uint led_value = to_rgb(hex);
    for (uint i = 0; i < PIXELS; i++) {
        pio_sm_put_blocking(pio, sm, led_value);
    }
}